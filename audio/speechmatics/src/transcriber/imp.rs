...
impl TranscriberSrcPad {
    ...
    fn loop_fn(&self) -> Result<(), gst::ErrorMessage> {
        let future = async move {
            ...
            let desynchronized =
                transcriber.imp().settings.lock().unwrap().latency_ms == std::u32::MAX;
...
impl Transcriber {
        ...
        match query.view_mut() {
            gst::QueryViewMut::Latency(ref mut q) => {
                ...
                if ret {
                    ...
                    let (live, our_latency) = if latency_ms == std::u32::MAX {
            ...
            gst::EventView::Segment(e) => {
                ...
                let (lateness, desynchronized) = {
                    ...
                    (
                        ...
                        settings.latency_ms == std::u32::MAX,
                    )
                };
    ...
    fn handle_buffer(
        ...
        let desynchronized = self.settings.lock().unwrap().latency_ms == std::u32::MAX;
    ...
    fn ensure_connection(&self) -> Result<(), gst::ErrorMessage> {
        ...
        let (connect_request, messages, desynchronized) = {
            let settings = self.settings.lock().unwrap();
            let desynchronized = settings.latency_ms == std::u32::MAX;
...
impl ElementImpl for Transcriber {
    ...
    fn provide_clock(&self) -> Option<gst::Clock> {
        if self.settings.lock().unwrap().latency_ms == std::u32::MAX {

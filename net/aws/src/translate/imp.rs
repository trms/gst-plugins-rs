...
impl Translate {
    ...
    fn sink_event(&self, pad: &gst::Pad, event: gst::Event) -> bool {
        ...
        match event.view() {
            ...
            Segment(e) => {
                let event = {
                    ...
                    let segment = if desynchronized {
                        ...
                        segment.set_position(self.obj().current_running_time().unwrap());
            ...
            Gap(_) => {
                ...
                if let Some(new_gap_event) = if desynchronized {
    ...
    async fn send(&self, to_translate: InputItems) -> Result<Vec<TranslateOutput>, Error> {
        ...
        let desynchronized = latency_ms == std::u32::MAX;
        ...
        for mut item in translated_items.drain(..) {
            ...
            if desynchronized {
                if let Some(start_pts) = start_pts {
                    item.pts = now
                        .unwrap()
                        .max(segment.position().unwrap_or(gst::ClockTime::ZERO))
                        + item.pts
                        - start_pts;
                } else {
